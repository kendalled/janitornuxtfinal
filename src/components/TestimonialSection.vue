<template>
  <section class="bg-blue-800 sm:pt-32 sm:pb-40 py-12">
    <h2
    ...
    <div class="container mx-auto sm:flex-row flex-col flex justify-center">
      <div v-for="q in reviews" :key="q.id" :class="[selected.includes(q.id) ? 'scaled bg-white' : 'desat bg-gray-300']" class="sm:w-1/3 max-w-sm sm:mb-0 mb-16 md:mx-2 lg:mx-6 shadow rounded pt-4 pb-10 px-12 transition-ease-in-out transition-transform transition-bg transition-250" @click="selected = [q.id]">
    ...
    <div class="inline-flex w-full justify-center mt-12">
      <button class="mx-4 bg-gray-100 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded focus:outline-none transition-bg" @click="moveTest(-1)">
        Prev
      </button>
      <button class="mx-4 bg-gray-100 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded focus:outline-none transition-bg" @click="moveTest(1)">
        Next
